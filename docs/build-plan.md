...
### M28 — 主流科研文档解析（PDF / DOCX / PPTX + 中英文 OCR）【第一优先】

- **目标**：升级 L0 文档抽取层，使代码项目之外、以论文和汇报材料为主的项目也能被可靠理解；不再把 PDF 二进制内容当作普通文本读取。
- **前置**：M3（项目理解）+ M27（Web 项目导入）。
- **新增文档解析依赖**：PDF 文本与页面渲染、DOCX、PPTX、中英文 OCR 所需依赖已由主控聊天确认；归入 `documents` 可选依赖，并由 `web` extra 一并安装。
- **产出**：升级 `papermine/extractor/` 文档抽取模块 + `scanner.py` 文档类型识别 + 单测与脱敏 fixture + Windows 安装说明 + 交接记录。
- **支持范围**：
  1. PDF：优先提取文本层；无文本或文本不足的页面转为图像，执行中英文 OCR。
  2. DOCX：提取段落、表格、页眉页脚，并对文档内嵌图片执行中英文 OCR。
  3. PPTX：按页提取文本、表格、演讲者备注，并对幻灯片内嵌图片执行中英文 OCR。
  4. 纯文本 / Markdown / LaTeX 保持原有读取能力；代码解析接口不变。
- **约束**：
  1. 不修改 `agents/understand.run(project_dir, dossier, llm)`、Dossier schema 或 M27 导入接口。
  2. 所有解析与 OCR 在本机 L0 层完成；传给 LLM 的内容仍遵守「结构化 facts + 脱敏后的必要片段」边界。
  3. 文档损坏、加密、解析依赖缺失或单页 OCR 失败时不得让整次分析崩溃；保留其他可读内容并给出可追溯提示。
  4. 解析有明确的字符、页面、像素和图片预算，避免异常文档导致内存或处理时间失控；达到预算时明确标记截断。
  5. Python 3.8 兼容；新增依赖必须使用兼容版本范围并可在 Windows 虚拟环境内通过 pip 安装，不要求用户另装 OCR 程序。
- **验收**：
  1. 文本型 PDF、扫描型中英文 PDF、含正文/表格/图片的 DOCX、含正文/表格/备注/图片的 PPTX 均能抽取到与原文一致的关键内容。
  2. PDF 不再按 UTF-8 二进制容错读取；损坏/加密文件安全降级，不影响同项目其他资产。
  3. 用以 PDF 论文为主的脱敏科研 fixture 回归，项目理解证据包含真实标题/摘要线索，不再由偶发二进制关键词主导。
  4. 文档抽取单测、全量 pytest、Web 前端构建、旧版冒烟测试通过；`papermine trace <run_id>` 满足 IDEATE ≤ 2、EVALUATE ≤ 3、LLM ≤ 50。

### M29 — 多源分层文献检索（每方向 8 篇目标）【第一优先】

- **目标**：解决研究方向文献数量偏少或零结果的问题；每个研究方向目标展示 8 篇、最多 12 篇真实论文，并明确区分「高度相关」与「部分相关」。
- **前置**：M5/M10（检索与相关性过滤）+ M16（批量/缓存/并行）+ M25（Literature & Gap 页面）。
- **检索源**：在 arXiv、Semantic Scholar 基础上增加 OpenAlex、Crossref、DBLP；均使用公开 API，不要求用户配置新密钥。
- **产出**：升级 `papermine/retrieval.py` 多源检索、分层召回和跨源去重；同步 Web 文献卡片与覆盖状态；新增单测、变更记录和交接记录。
- **兼容字段**：不修改 Dossier 顶层 schema；允许在现有 `literature[].papers[]` 条目内增加：
  ```json
  {
    "relevance_level": "high | partial",
    "match_reason": "命中的核心术语或扩展检索说明",
    "source": "arxiv | semantic_scholar | openalex | crossref | dblp",
    "source_records": ["..."],
    "doi": "..."
  }
  ```
- **召回策略**：
  1. 第一层使用完整研究问题和核心术语组合，筛选高度相关论文。
  2. 不足 8 篇时依次执行「核心任务 + 核心方法」「去除过窄限定词」「同义词/上位概念扩展」，候选标为部分相关。
  3. 多来源结果按 DOI、arXiv ID、规范化标题去重；合并来源记录，不把同一论文重复计数。
  4. 达到 8 篇后停止扩展，最终最多保留 12 篇；仍不足时如实返回实际数量并标记覆盖不足，不虚构、不复制凑数。
- **可靠性与性能约束**：
  1. 单个来源失败、超时或限流时继续其他来源，并在检索条目中记录成功来源。
  2. 缓存键需包含检索策略版本，避免继续命中旧版少量结果缓存。
  3. 不增加文献理解的逐篇 LLM 调用；继续使用 M15/M16 的批量、缓存与并行路径。
  4. 不提高查询改写 LLM 轮数；扩展查询优先使用确定性术语策略，避免 LLM 调用数量随来源数倍增。
- **Web 展示**：
  1. 每个 Query 显示「共 N 篇 / 目标 8 篇」和覆盖状态。
  2. 论文卡明确显示「高度相关」或「部分相关」、匹配原因和来源。
  3. 高度相关优先排序，部分相关用于补足背景证据；不再使用容易误解的英文 Relevant/Partial/Peripheral。
- **验收**：
  1. 对至少三个真实研究方向测试，其中包括原本零结果或少结果的方向；公开源可用时每方向达到 7–8 篇以上。
  2. 覆盖多源重复、单源失败、严格检索不足后扩展、最终仍不足四种路径。
  3. 全量 pytest、Web build、旧版冒烟通过；端到端 trace 保持 IDEATE ≤ 2、EVALUATE ≤ 3、LLM ≤ 50。
