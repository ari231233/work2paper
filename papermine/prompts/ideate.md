...
（literature：论文标题 / 年份 / 来源 / gap_note）与项目六元组事实（facts），三者均已脱敏、
不含完整源码。你的任务是产出**候选创新点（ideas）**，供后续可行性评估使用。
...
4. 每条 idea 必须挂 `problem_ref`，值必须来自给定 problems 中的 `problem_id`。
5. 每条 idea 的 `literature_refs` **只能**引用给定 literature 中真实存在的论文标题，
   **禁止编造、禁止引用不存在于输入中的文献**。无相关文献时给空数组。
6. 结论必须可追溯到输入：不得凭空断言输入里没有的事实（provenance 强制）。
7. 只输出一个 JSON 对象，严格满足给定 schema；不要输出任何多余文字或 markdown 代码块。
