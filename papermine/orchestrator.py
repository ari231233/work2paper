...
from . import experience, policy, reporting, storage, trace
...
def _render_report_md(dossier: Dossier) -> str:
    ...
    渲染逻辑已移至 ``papermine/reporting.py``（M23 报告重构的归属文件）；
    保留此入口名供 ``_write_report`` 与既有单测继续调用。
    """
    return reporting.render_report_md(dossier)
